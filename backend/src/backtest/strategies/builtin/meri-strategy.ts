...
export const meriStrategy: BacktestStrategy = {
  ...
  onCandle(ctx: CandleContext): Signal[] {
    ...
    // At transition from bucket N to N+1:
    //   idx5           = N+1 = CURRENT (still forming) 5m bucket
    //   signalIdx5     = N   = just-CLOSED bucket (SIGNAL BAR)
    //   prevSignalIdx5 = N-1 = bar before signal bar
    // All indicator reads are from signalIdx5 and earlier — zero look-ahead.
    ...
    // 15m indicators — read from the most recently CLOSED 15m bucket.
